...
async function getProductsVar(req,res){
    ...
    const stmt = "SELECT * FROM `selected_var` WHERE `product_id` = ? "

    return new Promise((resolve,reject)=>{
        db.query(stmt,[pro_id],function (err, result, fields) {
                if (err)
                    throw err;
                if (result.length == 0) {
                    console.log("No Item Variant");
                    reject(new Error("No var"));
                } else {
                    console.log(result)
                    resolve(result);
...
module.exports = {getAllProducts,getProductsVar}