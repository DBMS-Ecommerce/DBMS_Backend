...
class Product_Filter {
  ...
  static getAll_Products(sub_cat_id) {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT DISTINCT title FROM product Where sub_category_id=?",
        [sub_cat_id],
        (err, results) => {
          if (err) return reject(err);
          let products = [];
  ...
  static getAllSubCategories(cat_id) {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT DISTINCT title FROM sub_category Where category_id=?",
        [cat_id],
        (err, results) => {
          if (err) return reject(err);
          let sub_categories = [];
  ...
  static getAllSubCategoriesWithId(cat_id) {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT DISTINCT title,sub_category_id as id FROM sub_category Where category_id=?",
        [cat_id],
        (err, results) => {
          if (err) return reject(err);
          let sub_categories = [];
  ...
  static getAllProductsBySubCat(sub_cat_id) {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT DISTINCT title,product_id FROM product Where sub_category_id=?",
        [sub_cat_id],
        (err, results) => {
          if (err) return reject(err);
          let products = [];
